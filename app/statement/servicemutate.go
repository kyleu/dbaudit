...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Statement) error {
	if len(models) == 0 {
		return nil
	}
	q := database.SQLInsert(tableQuoted, columnsQuoted, len(models), s.db.Placeholder())
	vals := lo.FlatMap(models, func(arg *Statement, _ int) []any {
		return arg.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, vals...)
}

func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*Statement) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			logger.Infof("saving statements [%d-%d]", idx*chunkSize, ((idx+1)*chunkSize)-1)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Statement, logger util.Logger) error {
	q := database.SQLUpdate(tableQuoted, columnsQuoted, "\"id\" = @p19", s.db.Placeholder())
	data := model.ToData()
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Statement) error {
	if len(models) == 0 {
		return nil
	}
	q := database.SQLUpsert(tableQuoted, columnsQuoted, len(models), []string{"id"}, columnsQuoted, s.db.Placeholder())
	data := lo.FlatMap(models, func(model *Statement, _ int) []any {
		return model.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, data...)
}

func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*Statement) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			...
			logger.Infof("saving statements [%d-%d]", idx*chunkSize, count)
		}
