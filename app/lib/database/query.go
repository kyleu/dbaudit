...
func (s *Service) Select(ctx context.Context, dest any, q string, tx *sqlx.Tx, logger util.Logger, values ...any) error {
	...
	if tx == nil {
		...
		defer f(util.LengthAny(dest), "ran select query without transaction", err, util.ArrayFromAny[any](dest)...)
		return err
	}
	err = tx.SelectContext(ctx, dest, q, values...)
	defer f(util.LengthAny(dest), "ran select query with transaction", err, util.ArrayFromAny[any](dest)...)
	return err
}
